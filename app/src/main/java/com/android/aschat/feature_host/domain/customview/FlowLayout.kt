...
class FlowLayout(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) :
    ...
    private fun requestLayoutInner() {
        Handler(Looper.getMainLooper()).post(Runnable { requestLayout() })
    }

    fun <T>setAdapter(list: List<T>?, res: Int, mItemView: ItemView<T>) {
        if (list == null) {
            return
        }
        removeAllViews()
    ...
    abstract class ItemView <T> {
        abstract fun <T> getCover(item: T, holder: ViewHolder?, inflate: View?, position: Int)
    }
    ...
    inner class ViewHolder(var mConvertView: View) {
        ...
        fun setText(viewId: Int, text: Any?) {
            val view = getView<TextView>(viewId)!!
            view.text = text as CharSequence?
        }
    ...
    companion object {
        ...
        fun dipToPx(ctx: Context, dip: Float): Int {
            return TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dip, ctx.getResources().getDisplayMetrics()).toInt()
        }
    }
}